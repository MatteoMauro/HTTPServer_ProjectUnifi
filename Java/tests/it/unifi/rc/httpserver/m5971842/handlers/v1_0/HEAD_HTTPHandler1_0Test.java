 ...
 */
public class HEAD_HTTPHandler1_0Test {
	...
	/**
	 * Test if HEAD_HTTPHandler1_0 can retrieve correctly the files in this
	 * case: 1) host and root not specified (default -> host = null,
	 * root="/www"); 2) correct host specified; 3) root specified.
	 */
	@Test
	public void validRequest() {
		HEAD_HTTPHandler1_0 handlerDefault = setupHandler(null, null);
		HEAD_HTTPHandler1_0 handlerSpecifiedHost = setupHandler(validHost, null);
		HEAD_HTTPHandler1_0 handlerSpecifiedRoot = setupHandler(null, validRoot);
		HTTPRequest request = setupHTTPRequest(methodSamples + "head10.txt");
		// default
	...
	/**
	 * Test if HEAD_HTTPHandler1_0 returns 404 if file is not found.
	 */
	@Test
	public void fileNotFound404() {
		HEAD_HTTPHandler1_0 handlerDefault = setupHandler(null, null);
		HTTPRequest request = setupHTTPRequest(methodSamples + "head10FileNotFound.txt");
		HTTPReply reply = handlerDefault.handle(request);
	...
	public void hostNotCompatible() {
		...
		HEAD_HTTPHandler1_0 handler = setupHandler(validHost, null);
		HTTPRequest request = setupHTTPRequest(methodSamples + "head10WrongHost.txt");
	...
	public void methodNotCompatible() {
		HEAD_HTTPHandler1_0 handler = setupHandler(null, null);
		HTTPRequest request = setupHTTPRequest(methodSamples + "post10.txt");
		HTTPRequest request1 = setupHTTPRequest(methodSamples + "head11.txt");
		HTTPReply reply = handler.handle(request);
		// post10.txt is not the right method
		assertNull(reply);
		reply = handler.handle(request1);
		// head11.txt is not the right version
		assertNull(reply);
	}

	private HEAD_HTTPHandler1_0 setupHandler(String host, File root) {
		return new HEAD_HTTPHandler1_0(host, root);
	}
