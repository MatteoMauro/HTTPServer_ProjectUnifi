 ...
 */
public class POST_HTTPHandler1_0Test {
	...
	/**
	 * Test if POST_HTTPHandler1_0 can retrieve correctly the files in this
	 * case: 1) host and root not specified (default -> host = null,
	 * root="/www"); 2) correct host specified; 3) root specified.
	 */
	@Test
	public void validRequest() {
		POST_HTTPHandler1_0 handlerDefault = setupHandler(null, null);
		POST_HTTPHandler1_0 handlerSpecifiedHost = setupHandler(validHost, null);
		POST_HTTPHandler1_0 handlerSpecifiedRoot = setupHandler(null, validRoot);
	...
	/**
	 * Test if POST_HTTPHandler1_0 returns 404 if file is not found.
	 */
	@Test
	public void fileNotFound404() {
		POST_HTTPHandler1_0 handlerDefault = setupHandler(null, null);
		HTTPRequest request = setupHTTPRequest(methodSamples + "post10FileNotFound.txt");
		HTTPReply reply = handlerDefault.handle(request);
	...
	public void hostNotCompatible() {
		...
		POST_HTTPHandler1_0 handler = setupHandler(validHost, null);
		HTTPRequest request = setupHTTPRequest(methodSamples + "post10WrongHost.txt");
	...
	public void methodNotCompatible() {
		POST_HTTPHandler1_0 handler = setupHandler(null, null);
		HTTPRequest request = setupHTTPRequest(methodSamples + "get10.txt");
		HTTPRequest request1 = setupHTTPRequest(methodSamples + "post11.txt");
		HTTPReply reply = handler.handle(request);
		// get10.txt is not the right method
		assertNull(reply);
		reply = handler.handle(request1);
		// post11.txt is not the right version
		assertNull(reply);
	}

	private POST_HTTPHandler1_0 setupHandler(String host, File root) {
		return new POST_HTTPHandler1_0(host, root);
	}
